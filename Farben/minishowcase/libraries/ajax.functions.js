 ...
 */
	...
	function setGallery(id, link)
	{
		...
		if (active_id != id) {
			...
			innerhtml('gallery_title','<img src="'+indicator_src+'" /> <span>'+lang['loading_title']+'</span> <strong>'+imageName(galleryName(id))+'</strong>...');
	...
	function updateThumbs(request)
	{
		...
		gallery_title_cont = '<span class="xtra">'
			...
			+'</span> <strong>'
			+imageName(galleryName(active_id))
	...
	function closeImageWin()
	{
		...
		if (show_permalink) setCurrentPosition();
	...
	function thumbName(g)
	{
		...
		if (rows) {
			...
			if (g.length>dw) name = g.substring(0,dw-4)+'&#133;'+g.substring(g.length-3,g.length);
			else name = g;
		}
		return name;
	}
